...
var node_width = 5; // Set to panel_width later
...
function SceneNode(chars, start, duration, id, name) {
    ...
    this.height = 4; // Will be set later; proportional to link count
...
function add_char_scenes(chars, scenes, links, groups, panel_shift, comic_name) {
    ...
    for (var i = 0; i < chars.length; i++) {
	var s = new SceneNode([chars[i].id], [0], [1], [2]);
	s.char_node = true;
	s.y = i*text_height;
	s.x = 0;
	s.width = 5;
...
/**	var im = new Image();
    ...
    function mouseout(d) {
		d3.selectAll("[class=\"infobulle\"]").remove()
    }
...
function draw_chart(name, safe_name, folder, tie_breaker, center_sort, collapse) {
    d3.json(folder + ".json", function(j) {
	...
	for (var i = 0; i < jscenes.length; i++) {
	    ...
	    scenes[scenes.length] = new SceneNode(jscenes[i]['chars'], 
						  start, duration, 
						  parseInt(jscenes[i]['id']),jscenes[i]['name']);
...
draw_chart("Lucky Luke #38, \"Ma Dalton\"", "lucky", "lucky", true, false, false);
