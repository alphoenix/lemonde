...
var donnees = []; 
var doc = new Gselper({
	key: "0AiWUhxLpQgUXdFl5RDNMSUtqS0pnSWh2dVlUMnY3Smc",
	worksheet: "od6",
	...
	onComplete: function() {
		...
		var graph = (function() {
			...
			function load(callback){
					...
					z = d3.scale.ordinal()
					.domain(["NI","CRC","PG","ECOLO","RDSE","SOC","MoDem","NC","RS","UDI-UC","UMP","DLR","FN"])
					.range(["#999999","#d40000","#d40000","#608a32","#f96996","#d41066","#f56a20","#5bc1f4","#2e91db","#2e91db","#2c59a8","#7a0177","#04103f"])
			...
			function launch() {
				...
				.attr("stroke-width", 2)
				.attr("stroke", function(d) { return d3.rgb(z(d.groupe)).darker();})
				...
				.duration(1000)
				.attr("r", function(d) { return r(d.taille/5); });
				...
				function loadGravity(generator) {
					force
					.gravity(gravity)
					.charge( function(d) {return g(d.taille/5);})
					.friction(friction)
					.on("tick", function(e) {
						generator(e.alpha);
						node
					...
					function moveCenter(alpha) {
						force.nodes().forEach(function(d) {
							if(d.vote == "pour") {
								...
								d.x = d.x + (center_hesite.x - d.x) * (damper + 0.02) * alpha;
								d.y = d.y + (center_hesite.y - d.y) * (damper + 0.02) * alpha;
							}
							else if (d.vote == "nppv") {
				...
				function highlight(data,i,element ) {
					...
					if(data.taille > 1) content += ' (' + data.taille +' sénateurs)';
					else content += ' (' + data.groupe +')'
					content += '</span>';
				...
				function downlight( data, i, element ) {
					d3.select(element).attr("stroke", function(d) { return d3.rgb(z( d.groupe)).darker(); });
				}
				...
				graph.init(function() {
					$("#sum_pour").html("("+sum_pour+" sénateurs)");
					$("#sum_contre").html("("+sum_contre+" sénateurs)");
				});
